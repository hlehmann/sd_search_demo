define(['jquery', 'backbone', 'conf', 'i18n!nls/labels'],
  function($, Backbone, conf, labels) {
    return Backbone.Collection.extend({
      ...
      createDataStream: function(cb) {
        var self = this;
        $.ajax({
          url        : conf.inApiURL + 'stream',
          type       : 'POST',
          headers    : {
            login   : self.user,
            password: self.password
          ...
          .done(function(data) {
            self.streamId = data._id;
            cb(null, self.streamId);
          })
          .fail(function(jqXHR, textStatus, errorThrown) {
            cb(new Error(labels.result.dataStreamCreationFailed + ' ' + errorThrown));
          })
      }
